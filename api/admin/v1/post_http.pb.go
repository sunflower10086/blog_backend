...
const OperationPosterCreatePost = "/api.admin.v1.Poster/CreatePost"
const OperationPosterDeletePost = "/api.admin.v1.Poster/DeletePost"
const OperationPosterGetPost = "/api.admin.v1.Poster/GetPost"
const OperationPosterListPost = "/api.admin.v1.Poster/ListPost"
const OperationPosterUpdatePost = "/api.admin.v1.Poster/UpdatePost"
...
func RegisterPosterHTTPServer(s *http.Server, srv PosterHTTPServer) {
	...
	r.GET("/api/v1/post", _Poster_ListPost0_HTTP_Handler(srv))
}
...
func _Poster_ListPost0_HTTP_Handler(srv PosterHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in ListPostsRequest
		if err := ctx.BindQuery(&in); err != nil {
			return err
		}
		http.SetOperation(ctx, OperationPosterListPost)
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.ListPost(ctx, req.(*ListPostsRequest))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
...
type PosterHTTPClient interface {
	...
	ListPost(ctx context.Context, req *ListPostsRequest, opts ...http.CallOption) (rsp *ListPostsResponse, err error)
...
func (c *PosterHTTPClientImpl) ListPost(ctx context.Context, in *ListPostsRequest, opts ...http.CallOption) (*ListPostsResponse, error) {
	var out ListPostsResponse
	pattern := "/api/v1/post"
	path := binding.EncodeURL(pattern, in, true)
	opts = append(opts, http.Operation(OperationPosterListPost))
	opts = append(opts, http.PathTemplate(pattern))
	err := c.cc.Invoke(ctx, "GET", path, nil, &out, opts...)
	if err != nil {
		return nil, err
